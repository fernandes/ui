...
**Class**: `UI::Calendar::CalendarComponent`
...
```ruby
<%= render UI::Calendar::CalendarComponent.new(selected: Date.today) %>
```
...
```ruby
<%= render UI::Calendar::CalendarComponent.new(number_of_months: 2) %>
```
...
```ruby
<%= render UI::Calendar::CalendarComponent.new(week_starts_on: 1) %>
```
...
```ruby
<%= render UI::Calendar::CalendarComponent.new(fixed_weeks: true) %>
```

### Hide Outside Days

```ruby
<%= render UI::Calendar::CalendarComponent.new(show_outside_days: false) %>
```

### Start from Specific Month

```ruby
<%= render UI::Calendar::CalendarComponent.new(month: Date.new(2024, 12, 1)) %>
```
...
### Wrong: Using Phlex class (missing Component suffix)
...
### Correct: Use CalendarComponent

```ruby
<%= render UI::Calendar::CalendarComponent.new %>
```

### Wrong: Using ERB partial path
...
### Correct: Use ViewComponent class
