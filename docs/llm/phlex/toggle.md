...
```ruby
<%= render UI::Toggle::Toggle.new { "Toggle" } %>
```

## Component Path

- **Class**: `UI::Toggle::Toggle`
- **File**: `app/components/ui/toggle/toggle.rb`
...
```ruby
<%= render UI::Toggle::Toggle.new(variant: :outline) do %>
...
```ruby
<%= render UI::Toggle::Toggle.new do %>
...
```ruby
<%= render UI::Toggle::Toggle.new(pressed: true) do %>
  <svg class="h-4 w-4"><!-- bookmark icon --></svg>
<% end %>
```
...
```ruby
<%= render UI::Toggle::Toggle.new(variant: :outline) { "Outline Toggle" } %>
```

### With Size

```ruby
<%= render UI::Toggle::Toggle.new(size: :sm) { "Small" } %>
<%= render UI::Toggle::Toggle.new(size: :lg) { "Large" } %>
```

### Disabled State

```ruby
<%= render UI::Toggle::Toggle.new(disabled: true) do %>
...
```ruby
<%= render UI::Toggle::Toggle.new(classes: "text-muted-foreground") do %>
  Custom
<% end %>
```
...
<div class="flex gap-1">
  <%= render UI::Toggle::Toggle.new(variant: :outline, attributes: { "aria-label": "Toggle bold" }) do %>
    <svg class="h-4 w-4"><!-- bold icon --></svg>
  <% end %>

  <%= render UI::Toggle::Toggle.new(variant: :outline, attributes: { "aria-label": "Toggle italic" }) do %>
    <svg class="h-4 w-4"><!-- italic icon --></svg>
  <% end %>

  <%= render UI::Toggle::Toggle.new(variant: :outline, attributes: { "aria-label": "Toggle underline" }) do %>
    <svg class="h-4 w-4"><!-- underline icon --></svg>
  <% end %>
</div>
```
...
<div class="flex items-center gap-1 rounded-md border p-1">
  <%= render UI::Toggle::Toggle.new(variant: :default, size: :sm) do %>
    <svg><!-- icon --></svg>
  <% end %>
...
### ❌ Wrong: Missing nested Toggle class
...
```ruby
<%= render UI::Toggle::Toggle.new { "Text" } %>  # Correct!
```

### ❌ Wrong: Passing content as parameter

```ruby
<%= render UI::Toggle::Toggle.new(content: "Text") %>  # Won't work
```
...
```ruby
<%= render UI::Toggle::Toggle.new(variant: :primary) %>  # Invalid variant
```

### ✅ Correct: Use valid variants

```ruby
<%= render UI::Toggle::Toggle.new(variant: :default) %>  # or :outline
```
