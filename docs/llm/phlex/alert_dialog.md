...
**Example:**
```ruby
render UI::AlertDialog::AlertDialog.new(open: false, close_on_escape: true) do
...
Wrapper that opens the alert dialog when clicked. Does not directly render a button—it wraps interactive elements. By default, yields a block containing content or uses `as_child` composition to pass attributes to a child component.
...
**Example:**
```ruby
# Default: wrap a button directly
render UI::AlertDialog::Trigger.new do
  render UI::Button::Button.new { "Open Alert" }
end
```
...
```ruby
render UI::AlertDialog::Content.new do
  render UI::AlertDialog::Header.new { "Delete Account?" }
end
```
...
render UI::AlertDialog::Header.new do
  ...
  render UI::AlertDialog::Description.new { "This cannot be undone." }
end
```
...
render UI::AlertDialog::Action.new(variant: :destructive) { "Delete Account" }

# Custom size
render UI::AlertDialog::Action.new(size: :lg) { "Continue" }
```
...
**Example:**
```ruby
# Standard cancel button (outline variant by default)
render UI::AlertDialog::Cancel.new { "Cancel" }

# Override to be a ghost button
render UI::AlertDialog::Cancel.new(variant: :ghost) { "Cancel" }
```
...
**Keyboard Navigation:**
- `Enter` - Activate the focused button (Action or Cancel)
- `Escape` - Close the dialog (if `close_on_escape: true`)
- `Tab` - Move focus between buttons
- `Shift+Tab` - Move focus backwards between buttons

**ARIA Attributes:**
- `role="alertdialog"` - On the content container, identifies this as an alert dialog
- `aria-describedby` - Automatically linked to description for screen readers
- `aria-labelledby` - Automatically linked to title for screen readers

**Screen Reader Behavior:**
- Content is announced as an "alert dialog"
- Title and description are read first
- Focus is automatically managed within the dialog
- Dialog closing is announced when triggered
...
```ruby
# This won't work - Trigger needs something interactive
render UI::AlertDialog::Trigger.new { "Open" }
```

**Why it's wrong:** Trigger is a container. It needs a button or interactive element inside to be clickable.
...
render UI::AlertDialog::Trigger.new do
  tag.a(href: "#", class: "underline") { "Click here" }
end
```
...
```ruby
render UI::AlertDialog::Action.new(
  variant: :destructive,
  size: :lg
) { "Delete Everything" }
```
...
```ruby
render UI::AlertDialog::Trigger.new do
  tag.a(href: "#", class: "text-blue-600 underline hover:text-blue-800") do
    "Remove Item"
  end
end
```
...
render UI::AlertDialog::AlertDialog.new do
  render UI::AlertDialog::Trigger.new do
    render UI::Button::Button.new(type: :submit) { "Submit" }
  end
...
- ERB implementation: `docs/llm/erb/alert_dialog.md`
- Pattern docs: `docs/patterns/as_child.md` (if implementing asChild in future)
