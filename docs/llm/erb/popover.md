# Popover (ERB)

Displays rich content in a floating panel, triggered by a button.

## Component Paths

- **Root**: `ui/popover/popover`
- **Trigger**: `ui/popover/popover_trigger`
- **Content**: `ui/popover/popover_content`

## Installation

The Popover component requires `@floating-ui/dom` for positioning:

```json
{
  "dependencies": {
    "@floating-ui/dom": "^1.7.4"
  }
}
```

## Basic Usage

```erb
<%= render "ui/popover/popover" do %>
  <%= render "ui/popover/popover_trigger" do %>
    <%= render "ui/button/button", variant: "outline" do %>
      Open popover
    <% end %>
  <% end %>
  <%= render "ui/popover/popover_content" do %>
    <div class="grid gap-4">
      <div class="space-y-2">
        <h4 class="font-medium leading-none">Dimensions</h4>
        <p class="text-sm text-muted-foreground">Set the dimensions for the layer.</p>
      </div>
      <div class="grid gap-2">
        <div class="grid grid-cols-3 items-center gap-4">
          <%= render "ui/label/label", for: "width" do %>Width<% end %>
          <%= render "ui/input/input", id: "width", value: "100%", classes: "col-span-2 h-8" %>
        </div>
        <div class="grid grid-cols-3 items-center gap-4">
          <%= render "ui/label/label", for: "height" do %>Height<% end %>
          <%= render "ui/input/input", id: "height", value: "25px", classes: "col-span-2 h-8" %>
        </div>
      </div>
    </div>
  <% end %>
<% end %>
```

## Component Structure

### Popover (Root)

Container component that provides context for the popover.

**Parameters:**

- `placement` (String, default: `"bottom"`) - Position relative to trigger
  - Values: `"top"`, `"bottom"`, `"left"`, `"right"`, `"top-start"`, `"top-end"`, `"bottom-start"`, `"bottom-end"`, `"left-start"`, `"left-end"`, `"right-start"`, `"right-end"`
- `offset` (Integer, default: `8`) - Distance in pixels from trigger
- `trigger` (String, default: `"click"`) - Interaction type
  - Values: `"click"`, `"hover"`, `"manual"`
- `hover_delay` (Integer, default: `0`) - Delay in ms for hover trigger
- `align` (String, default: `nil`) - Legacy parameter for alignment (use placement instead)
- `classes` (String, default: `""`) - Additional CSS classes
- `attributes` (Hash, default: `{}`) - Additional HTML attributes

**Example with custom placement:**

```erb
<%= render "ui/popover/popover", placement: "top-start", offset: 8 do %>
  <%# ... trigger and content %>
<% end %>
```

### Trigger

Button or element that triggers the popover.

**Parameters:**

- `classes` (String, default: `""`) - Additional CSS classes
- `attributes` (Hash, default: `{}`) - Additional HTML attributes

**Example:**

```erb
<%= render "ui/popover/popover_trigger" do %>
  <%= render "ui/button/button", variant: "outline" do %>
    Open
  <% end %>
<% end %>
```

**Example with custom classes:**

```erb
<%= render "ui/popover/popover_trigger", classes: "custom-trigger" do %>
  <button class="custom-button">Open</button>
<% end %>
```

### Content

The floating content panel.

**Parameters:**

- `classes` (String, default: `""`) - Additional CSS classes
- `attributes` (Hash, default: `{}`) - Additional HTML attributes

**Default styling:**
- Width: `w-72` (18rem / 288px)
- Can be overridden with `classes` parameter

**Example with custom width:**

```erb
<%= render "ui/popover/popover_content", classes: "w-80" do %>
  <%# Content %>
<% end %>
```

## Common Patterns

### With Form Controls

```erb
<%= render "ui/popover/popover" do %>
  <%= render "ui/popover/popover_trigger" do %>
    <%= render "ui/button/button", variant: "outline" do %>
      Settings
    <% end %>
  <% end %>
  <%= render "ui/popover/popover_content", classes: "w-80" do %>
    <div class="grid gap-4">
      <div class="space-y-2">
        <h4 class="font-medium leading-none">Settings</h4>
        <p class="text-sm text-muted-foreground">Configure your preferences.</p>
      </div>
      <div class="grid gap-2">
        <%# Form controls here %>
      </div>
    </div>
  <% end %>
<% end %>
```

### Hover Trigger

```erb
<%= render "ui/popover/popover", trigger: "hover", hover_delay: 300 do %>
  <%= render "ui/popover/popover_trigger" do %>
    Hover over me
  <% end %>
  <%= render "ui/popover/popover_content" do %>
    This appears on hover
  <% end %>
<% end %>
```

### Manual Control

```erb
<%# Trigger popover programmatically from JavaScript %>
<%= render "ui/popover/popover", trigger: "manual", attributes: { data: { popover_open_value: "false" } } do %>
  <%= render "ui/popover/popover_trigger" do %>
    <button data-action="click->my-controller#openPopover">Manual trigger</button>
  <% end %>
  <%= render "ui/popover/popover_content" do %>
    Manually controlled content
  <% end %>
<% end %>
```

### With Icon Button Trigger

```erb
<%= render "ui/popover/popover" do %>
  <%= render "ui/popover/popover_trigger" do %>
    <%= render "ui/button/button", variant: "ghost", size: "icon" do %>
      <svg class="w-4 h-4">...</svg>
    <% end %>
  <% end %>
  <%= render "ui/popover/popover_content" do %>
    Content here
  <% end %>
<% end %>
```

## Accessibility

The component includes:

- Focus management with Escape key to close
- Click outside to dismiss
- Proper ARIA attributes via `data-state` attribute
- Keyboard navigation support

## Positioning

Uses Floating UI for smart positioning:

- Automatically flips when near viewport edges
- Shifts to stay within viewport
- Updates position on scroll/resize
- Supports all standard placements

## Animation

Content animates in/out using tw-animate-css:

- Fade in/out
- Zoom scale (95-100%)
- Slide direction based on `data-side` attribute

Animation is controlled via `data-state` attribute (managed by Stimulus controller).

## Error Prevention

**Common Mistakes:**

❌ **WRONG - Incorrect partial path:**
```erb
<%= render "popover" %>  # Missing ui/ namespace
<%= render "ui/popover" %>  # Incomplete path
```

✅ **CORRECT - Full partial path:**
```erb
<%= render "ui/popover/popover" %>
```

❌ **WRONG - Missing trigger or content:**
```erb
<%= render "ui/popover/popover" do %>
  <%# Empty - needs trigger and content %>
<% end %>
```

✅ **CORRECT - Include both trigger and content:**
```erb
<%= render "ui/popover/popover" do %>
  <%= render "ui/popover/popover_trigger" do %>...<% end %>
  <%= render "ui/popover/popover_content" do %>...<% end %>
<% end %>
```

## Related Components

- Button - Often used as trigger
- Label - For form fields within popover
- Input - For form controls
- Tooltip - For simpler hover content
